...
class DNSResolver(object):
    def __init__(self, loop):
        self.timeout = 3.0
        self._channel = pyuv.cares.Channel(timeout=self.timeout, sock_state_cb=self._sock_state_cb)
        self.loop = loop
    ...
    def _timer_cb(self, timer):
        self._channel.process_fd(pyuv.cares.ARES_SOCKET_BAD, pyuv.cares.ARES_SOCKET_BAD)
    ...
    def _poll_cb(self, handle, events, error):
        ...
        if not events & pyuv.UV_READABLE:
            read_fd = pyuv.cares.ARES_SOCKET_BAD
        if not events & pyuv.UV_WRITABLE:
            write_fd = pyuv.cares.ARES_SOCKET_BAD
        self._channel.process_fd(read_fd, write_fd)
